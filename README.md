...
DND Mode also suppresses normal chat and unrelated controls. Discord display names become character names, while immutable Discord user IDs preserve levels and stats after name changes. Jangle begins with a small easy problem, escalates through three scenes, and uses fresh AI narration grounded in the saved journal and lasting campaign facts. On a character's turn, speak an action without the wake word. Straightforward roleplay resolves immediately; uncertain or risky actions use one check, and that player says `roll`. Attacks use one simple current-threat health total so a hit deals rolled damage instead of granting any outcome the player claims. Dice, HP, XP, levels, damage, and success or failure are calculated by code and cannot be changed by the model. Laughter, short interjections, and speech captured before a turn opens are ignored.
